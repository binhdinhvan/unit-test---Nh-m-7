...
async function createExcel() {
  ...
  // Danh sách các Test Bug từ code
  const failedTests = [
    'TC_PTF_008', 'TC_PTF_020', 'TC_PTF_021', 'TC_PTF_025', 
    'TC_PTF_026', 'TC_PTF_028', 'TC_PTF_032'
  ];

  const content = fs.readFileSync(
    path.join(__dirname, '../MaiHieu/src/test/java/com/mxhieu/doantotnghiep/PracticeTestFeatureTests.java'), 
    'utf8'
  );
  ...
  while ((match = regex.exec(content)) !== null) {
    ...
    let usecase = 'Quản lý bài thi hiển thị';
    let clazz = 'TestServiceImpl';
    if(numId >= 22) { usecase = 'Làm bài và lưu kết quả'; clazz = 'TestAttemptServiceImpl'; }
  ...
  const outputPath = 'Unit_Testing_Report_PracticeTest.xlsx';
